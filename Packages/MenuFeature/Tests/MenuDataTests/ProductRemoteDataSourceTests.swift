...
func productRemoteSourceMapsFailures() async {
    ...
    do { _ = try await unauthorized.detail(productID: "p1"); Issue.record("expected unauthorized") }
    catch let error as ProductRemoteDataSourceError { #expect(error == .unauthorized) }
...
func productRemoteSourceCancellation() async {
    ...
    catch let error as ProductRemoteDataSourceError { #expect(error == .cancelled) }
